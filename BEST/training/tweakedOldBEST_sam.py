...
def train(h5Dir, modelFile, plotDir, suffix, userPatience, maskPath, TrnValTstEvents):
    ...
    myMask = [True if str(i) in maskIndex else False for i in range(80)]
    ...
    myTrainEvents      = [np.array(h5py.File(h5Dir+mySample+"Sample_2018_BESTinputs_train_flattened_standardized.h5",     "r")["BES_vars"])[:,myMask] for mySample in sampleTypes]
    myValidationEvents = [np.array(h5py.File(h5Dir+mySample+"Sample_2018_BESTinputs_validation_flattened_standardized.h5","r")["BES_vars"])[:,myMask] for mySample in sampleTypes]
