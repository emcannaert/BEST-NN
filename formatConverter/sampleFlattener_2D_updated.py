...
## different versson of flattenFile that makes sure each bin has the EXACT same number of events 
## VERY inefficienct, might not work with more than 35 bins

def flattenFileNew(min_events, nEvents, h5Dir, outDir, sampleTypes, year, setType, bins_SJ, bins_HT, userBatchSize, mass_type, args):
    print("Begin flattening for", setType)
    ...
    for sample in sampleTypes:
        ...
        sjMassData = np.array(fIn["BES_vars"][:, 88])
        htData = np.array(fIn["BES_vars"][:, 111])
        ...
        for myKey in fIn.keys():
            ...
            for i in range(0,len(bins_SJ)-1):
                for j in range(0, len(bins_HT)-1):
                    ...
                    ## this replaces the original train_test_split
                    result_old = myKeyData[combinedBool]
                    ...
                    #print("int(  min(     nEvents[0][i][j],nEvents[1][i][j] )     ) = %s"%(int(  min(     nEvents[0][i][j],nEvents[1][i][j] )     )))
                    #print("int( nEvents[0][i][j]) = %s, int( nEvents[1][i][j]) = %s"%( int(nEvents[0][i][j]), int(nEvents[1][i][j])) )


                    #assert len(result) == events_to_keep, "ERROR: len(result) == events_to_keep, len(result) = %s, events_to_keep = %s"%(len(result), events_to_keep)
...
def getProbabilities2D(h5Dir, sampleTypes, year, setType, mass_type,args, bins_HT, bins_SJ):
    ...
    for sample in sampleTypes:
        ...
        hist2d, _, _ = np.histogram2d(HT_values, SJ_mass_values, bins=[bins_HT,bins_SJ])
...
## return 2D array that gives the minimum number of events that between training categories in a given bin
### 

def getMinNEventsPerBin(h5Dir, sampleTypes, year, setType, mass_type,args, bins_HT, bins_SJ):
    nEvents = []
    probs  = []
    ...
    min_events = np.minimum(nEvents[0], nEvents[1])  # gives NxM array of the minimum events between samples
    print("======== MIN EVENTS ARE ============ ")
    print_matrix(min_events,0,1)

    return min_events, nEvents ##  [signal_probs, BR_probs]
...
def print_matrix(matrix, precision=2, cell_width=6):
    ...
    for row in matrix:
        formatted_row = " ".join( fmt_float%elem if isinstance(elem, float) else str(elem).rjust(cell_width) for elem in row )
        print(formatted_row)
...
def printPostFlatNEvents(h5Dir, sampleTypes, year, setType, mass_type,args, bins_HT, bins_SJ):
    ...
    for sample in sampleTypes:
        ...
        hist_2d, _, _ = np.histogram2d([], [], bins=[bins_HT, bins_SJ]) ## ERROR WARNING: could be a problem if HT and SJ mass have different number of bins
        nEvents.append( hist_2d )
    ...
    print("Number of POST-FLAT signal events AFTER REOPENING FILE: ")
    print_matrix(nEvents[0],0,1)

    print("Number of POST-FLAT background events AFTER REOPENING FILE: ")
    print_matrix(nEvents[1],0,1)
