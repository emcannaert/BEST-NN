...
for part, partValues in partDict.items():
    ...
    for partmass, partmassValues in partValues[2].items():
        ...
        ak8HistDict[partmass] = ROOT.THStack(partmass,partmass)

# print(Dictionary)
# Now particle dictionary has the form: 
# {particle:[restmass, restmass range, {mass point:[root file name, root TFile object, {coarse key:THStack Coarse scan object, fine key:THStack Fine scan object} ] } ], ... }
...
for labvar, binInfo in labFrameDict.items(): # This iterates over the lab frame variables in the root file
    ...
    # labvarFirstLoop = True
    for part, partValues in partDict.items(): # Iterate over each particle
        for partmass, partmassValues in partValues[2].items():
            ...
            if (labvar == "jetAK8_mass") or (labvar == "jetAK8_SoftDropMass"):
                ...
                ak8HistDict[partmass].Add(htemp.Clone(labvar))
            else:
...
ROOT.gStyle.SetPalette(ROOT.kCool) #109 kCool palette for indvidual plots
for partmass, stack in ak8HistDict.items():
...
# These will replace the very large dictionaries, will delete dictionaries
partmassList = ["all"]
...
for part, partValues in partDict.items(): # Iterate over each particle
    for partmass, partmassValues in partValues[2].items():
        ...
        for var in varDict.keys(): # Iterate over each variable
            ...
            del fineStack
        partmassValues[1].Close() # Close all of the root files
        print(partmassValues[0]+" closed.")    
