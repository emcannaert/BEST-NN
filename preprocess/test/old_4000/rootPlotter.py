...
for part, partValues in Dictionary["PARTICLES"].items():
    ...
    for var, varValues in Dictionary["VARIABLES"].items(): # This iterates over the variables in the root file
        partValues[3][var+part+"_Coarse_Scan"] = ROOT.THStack(var+part+"_Coarse_Scan",var+part+"_Coarse_Scan") # Coarse Scan Stack [3][key] for each particle for each variable
        partValues[3][var+part+"_Fine_Scan"]   = ROOT.THStack(var+part+"_Fine_Scan",var+part+"_Fine_Scan")    # Fine Scan Stack [3][key] for each particle for each variable
    ak8MassDict[part] = ROOT.THStack(part,part)
...
# # Plot ak8 stuff:
# ak8Path = plotPath+"ak8/" 
# if not os.path.exists(ak8Path): os.makedirs(ak8Path) # If directory doesn't exist, create it        
# for part, partValues in Dictionary["PARTICLES"].items(): # Iterate over each particle
#     partStack = ROOT.THStack("ak8_"+part,"ak8_"+part) # 
...
#     for ak8var, ak8stack in ak8vars.items():
#         jettree.Draw(ak8var, "", "norm") # Create histogram
#         htemp = ROOT.gROOT.FindObject("htemp") # Grab histogram, it is named "htemp" by default, and overwritten each time 
#         partStack.Add(htemp.Clone(ak8var)) # Clone histogram, add to Stack
#         htemp.SetTitle(part)
#         ak8stack.Add(htemp.Clone(part))
#         del htemp
#     partak8canvas = ROOT.TCanvas("ak8_"+part)
#     partak8canvas.cd()
#     partStack.Draw("PMC PLC HIST NOSTACK")
#     partStack.GetXaxis().SetRangeUser(0,600)
#     partak8legend = ROOT.gPad.BuildLegend(0.5,1.,1.) # Create legend at coords
#     partak8legend.SetNColumns(2) # Set legend columns to 2
#     ROOT.gPad.Update() # Draw legend
#     partak8canvas.SaveAs(ak8Path+"ak8_"+part+".png")
#     partak8canvas.Close()

# ROOT.gStyle.SetPalette(ROOT.kRainBow) # kRainBow palette; bad for 2D data but fine here 
# for ak8var, ak8stack in ak8vars.items():
#     ak8Canvas = ROOT.TCanvas(ak8var) # Create canvas
#     ak8Canvas.cd() # Switch to new canvas
#     ak8stack.Draw("PMC PLC HIST NOSTACK") # Draw all histos in Stack at once. PMC/PLC for auto colors, NOSTACK to keep histos separate
#     ak8stack.GetXaxis().SetRangeUser(0,600)
#     ak8legend = ROOT.gPad.BuildLegend(0.5,1.,1.) # Create legend at coords
#     ak8legend.SetNColumns(3) # Set legend columns to 2
#     ROOT.gPad.Update() # Draw legend
#     ak8Canvas.SaveAs(ak8Path+ak8var+".png") # Save plot as png 
...
for labvar in labFrameDict: # This iterates over the lab frame variables in the root file
    ...
    for part, partValues in Dictionary["PARTICLES"].items(): # Iterate over each particle
        ...
        elif labvar == "nJets":             jettree.Draw(labvar+">>htemp(50,0,50)", "", "norm") # Create histogram
        ...
        if (labvar == "jetAK8_mass") or (labvar == "jetAK8_SoftDropMass"): 
            ak8MassDict[part].Add(htemp.Clone(part)) # Add to ak8 mass plots (plot masses together)
        else:
            indvLabString = labvar+"_"+part
            indvLabCanvas = ROOT.TCanvas(indvLabString)
            indvLabCanvas.cd()
            indvLabHist = htemp.Clone(indvLabString)
            indvLabHist.SetTitle(indvLabString)
            indvLabHist.SetStats(0) # Hide stats box
            indvLabHist.Draw("HIST") # HIST needed bc of normalization
            indvLabCanvas.SaveAs(labPath+indvLabString+".png") # Save plot as png 
            indvLabCanvas.Close() # Close canvas now that we are done

        htemp.SetTitle(part) # Change hist title to the current particle, which updates the all legend entry later
        labStack.Add(htemp.Clone(part)) # Clone histogram, add to Stack for current labvar
    ...
    labLegend.SetNColumns(3) # Set legend columns to 3
    ROOT.gPad.Update() # Draw legend
    if (labvar == "jetAK8_mass") or (labvar == "jetAK8_SoftDropMass"): saveString = labPath+"ak8_masses/"+labvar+"_all.png"
    else:                                                              saveString = labPath+labvar+"_all.png"
    labCanvas.SaveAs(saveString) # Save plot as png 
    labCanvas.Close() # Close canvas now that we are done

ROOT.gStyle.SetPalette(ROOT.kCool) #109 kCool palette for indvidual plots
for part, stack in ak8MassDict.items():
    ak8Canvas = ROOT.TCanvas("ak8_Masses_"+part) # Create canvas
    ak8Canvas.cd() # Switch to new canvas
    ...
    ROOT.gPad.Update() # Draw legend
    ak8Canvas.SaveAs(labPath+"ak8_masses/"+part+".png") # Save plot as png 
    ak8Canvas.Close() # Close canvas now that we are done
...
for boost in boosts: # Iterate over boosts
    ...
    for var, varValues in Dictionary["VARIABLES"].items(): # This iterates over the variables in the root file
        boostString = var+str(boost)+"GeV" # Name of the histogram in each root file for this variable for this boost
        boostStack = ROOT.THStack(boostString,boostString) # THStack object for this variable for this boost (fill with 6 particles)
        ...
        for part, partValues in Dictionary["PARTICLES"].items(): # Iterate over each particle
            ...
            coarseStack = partValues[3][var+part+"_Coarse_Scan"] # THStack object for Coarse Scan for this variable for this particle
            fineStack   = partValues[3][var+part+"_Fine_Scan"] # THStack object for Fine Scan for this variable for this particle
            
            # Conditions that trigger the fine and coarse scans:
            trigCoarse = True if             (boost > 40) and ((boost % 15) == 0)            else False # Fill Coarse Scan every 15 GeV, starting with 30 GeV (so 30, 45, 60,...)
            ...
            # Save individual plots
            indvPath = plotPath+"boosts/"+var[:-1]+"/"+part+"/"
            if not os.path.exists(indvPath): os.makedirs(indvPath) # If directory doesn't exist, create it
            indvString = boostString+"_"+part
            indvCanvas = ROOT.TCanvas(indvString)
            indvCanvas.cd()
            indvHist = htemp.Clone(indvString)
            indvHist.SetTitle(indvString)
            indvHist.SetStats(0) # Hide stats box
            indvHist.Draw("HIST") # HIST needed bc of normalization
            # indvlegend = ROOT.gPad.BuildLegend(0.5,1.,1.) # Create legend at coords
            # ROOT.gPad.Update() # Draw legend
            indvCanvas.SaveAs(indvPath+indvString+".png") # Save plot as png 
            indvCanvas.Close() # Close canvas now that we are done

            # htemp.Scale(1./htemp.Integral()) # Normalize histogram to 1, giving the probability frequency for each bin

            htemp.SetTitle(part) # Change hist title to the current particle, which updates Boost legend entry later
            boostStack.Add(htemp.Clone(part)) # Clone histogram, add to Stack for current boost

            htemp.SetTitle(str(boost)+"GeV") # Now change the title to current boost, which updates Scan legend entries later
            if trigCoarse:  coarseStack.Add(htemp.Clone(str(boost)+"GeV"))  # Clone histogram, add to Stack for Coarse Scan
            if trigFine:    fineStack.Add(htemp.Clone(str(boost)+"GeV"))  # Clone histogram, add to Stack for Fine Scan
        ...
        blegend.SetNColumns(3) # Set legend columns to 2
        ROOT.gPad.Update() # Draw legend
...
for part, partValues in Dictionary["PARTICLES"].items(): # Iterate over each particle
    for var, varValues in Dictionary["VARIABLES"].items(): # Iterate over each variable
        ...
        coarseStack = partValues[3][var+part+"_Coarse_Scan"]
        coarseCanvas = ROOT.TCanvas(var+part+"_Coarse_Scan") # Create canvas
        ...
        coarselegend.SetNColumns(4) # Set legend columns to 3
        ROOT.gPad.Update() # Draw legend
        coarseCanvas.SaveAs(scanPath+var+part+"_Coarse_Scan"+".png") # Save plot as png
        coarseCanvas.Close() # Close canvas now that we are done

        fineStack = partValues[3][var+part+"_Fine_Scan"] 
        fineCanvas = ROOT.TCanvas(var+part+"_Fine_Scan") # Create canvas
        ...
        finelegend.SetNColumns(4) # Set legend columns to 3
        ROOT.gPad.Update() # Draw legend
        fineCanvas.SaveAs(scanPath+var+part+"_Fine_Scan"+".png") # Save plot as png
...
# Dictionary["PARTICLES"]["all"] = None
# for var, varValues in Dictionary["VARIABLES"].items(): # This iterates over the variables in the root file
#     print("Variable: " + var[:-1])
#     gifPath = plotPath+"boosts/"+var[:-1]+"/" # Unique directory for each variable, trimming the underscore
#     # gifFile = gifPath+var+"all.gif"
#     # if os.path.exists(gifFile): os.remove(gifFile) # Delete old gif file
#     # for boost in boosts: # Iterate over boosts
#     #     # boostString = var+str(boost)+"GeV" # Name of the histogram in each root file for this variable for this boost    
#     #     boostImage = ROOT.TImage.Open(gifPath+"all/"+var+str(boost)+"GeV.png")
#     #     if boost != 200:    boostImage.WriteImage(gifFile+"+30") # +50 is 50 centiseconds delay
#     #     else:               boostImage.WriteImage(gifFile+"++100++") # The final image needs to be written with the ++ to get it to infinite loop
#     #     del boostImage

#     for part, partValues in Dictionary["PARTICLES"].items(): # Iterate over each particle
#         gifFile = gifPath+var+part+".gif"
#         if os.path.exists(gifFile): os.remove(gifFile) # Delete old gif file
#         for boost in boosts: # Iterate over boosts
#             partImage = ROOT.TImage.Open(gifPath+part+"/"+var+str(boost)+"GeV_"+part+".png")
#             if boost != 200:    partImage.WriteImage(gifFile+"+25") # +50 is 50 centiseconds delay
