...
def makeCM(model_BEST, h5Dir, plotDir, suffix, maskPath, testMaxEvents, modelType, scalePath):
    ...
    myTestEvents = [np.array(h5py.File(h5Dir+mySample+"Sample_2017_BESTinputs_test_flattened.h5","r")["BES_vars"])[:,myMask] for mySample in sampleTypes]
    
    # myTestEvents = [np.array(h5py.File(h5Dir+mySample+"Sample_2017_BESTinputs_test_flattened.h5","r")["BES_vars"])[:,:] for mySample in sampleTypes]
    ...
    # print("Scaling test...")
    # scaledTest = [scaler.transform(testArr) for testArr in myTestEvents]

    # print("Applying mask...")
    # myTestEvents = [testArr[:,myMask] for testArr in scaledTest]
    # del scaledTest
    # del scaler   
    ...
    print("Scaling test...")
    globals()["jetBESvarsTest"] = scaler.transform(globals()["jetBESvarsTest"][:testMaxEvents])
    del scaler   

    # print("Applying mask...")
    # globals()["jetBESvarsTest"] = globals()["jetBESvarsTest"][:,myMask]
    ...
    classifylog = open("Logs/" + modelType + "_classifyLog.txt", "a") 
    classifylog.write("-----------------------------------\n")
...
if __name__ == "__main__":
    ...
    if os.path.isfile(modelFile):
        makeCM(load_model(modelFile), args.h5Dir, plotDir, mySuffix, maskSave, testMaxEvents, modelType, scalePath)
