...
def loadSampleTypes(args, sampleTypes, setTypes, maxEvents, mass_type, year='', testSet="flattened"):
    h5Dir = args.h5Dir
    scale = args.scale
    if year == '': year = args.year
    ...
    for mySet in setTypes:
        ...
        lastVarArrays = [np.array(h5py.File(h5Dir + mySample + h5Path, "r")["BES_vars"])[:numEvents,-1] for mySample in sampleTypes]
        lastVarArrays = np.array(lastVarArrays)
    lastVarArrays=np.concatenate(lastVarArrays)
        

    return lastVarArrays
...
def plotROC(BESpredict, truthLabels, plotDir, samples, modelType, suffix, year,sampleTypeArray):

    print("Plotting ROC curves")
    # samples = ["W", "Z", "Higgs", "Top", "Bottom", "QCD"]
    sampleTypes = ["allDecays_MChi-1000","allDecays_MChi-1500","allDecays_MChi-2000","allDecays_MChi-2500","allDecays_MChi-3000"]
    # saveDir = plotDir + "roc/" 
    ...
    tprBES = dict()
    roc_auc_BES = dict()
    print("truthLabel shape",truthLabels.shape)
    print("truthLabel",truthLabels)
    print("BESpredict, shape",BESpredict.shape)
    print("sampleTypeArray shape", sampleTypeArray.shape)
 
    # for i in range(n_classes):
    for i, sample in enumerate(sampleTypes):
        print("i is %i, sample is %s"%(i,sample))
        if sample == "allDecays_MChi-1000":
            signal_value = 4
        if sample == "allDecays_MChi-1500":
            signal_value = 5
        if sample == "allDecays_MChi-2000":
            signal_value = 6
        if sample == "allDecays_MChi-2500":
            signal_value = 7
        if sample == "allDecays_MChi-3000":
            signal_value = 8
        mask = (sampleTypeArray == signal_value) | (sampleTypeArray < 4)
        print("mask",mask)

        filtered_truthLabels = truthLabels.ravel()[mask]
        filtered_BESpredict = BESpredict.ravel()[mask]
        print("filtered_truthLabels",filtered_truthLabels)
        fprBES[sample], tprBES[sample], _ = roc_curve(filtered_truthLabels, filtered_BESpredict)
    ...
    all_fprBES = numpy.unique(numpy.concatenate([fprBES[sample] for sample in sampleTypes]) )
    ...
    path  = saveDir + "png/"  + '_ROCplot_%s.png'%year
    # path  = saveDir + "png/" + labelDict[key][1] + '_ROCplot_%s.png'%year
    plt.savefig(path)
    path  = saveDir + "pdf/" + '_ROCplot_%s.pdf'%year
...
def plotAll(args, strings, truthData, modelType, BESpredict, mass_type, year, samples,sampleTypeArray):
    ...
    print("sampleTypeArray",sampleTypeArray)
    plotROC(BESpredict, truthData, plotDir, samples, args.modelType, suffix, year,sampleTypeArray)
